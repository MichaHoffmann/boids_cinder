...
class TestSpecParser
{
    ...
    std::size_t m_start = std::string::npos, pos = 0;
    std::string m_arg;
...
public:
    class iterator
    {
        ...
        void calcLength ()
        {
          ...
          {
            size_t len = width;
            while ( len > 0 && !isBoundary( pos + len ))
              --len;
            while ( len > 0 && isWhitespace( line()[ pos + len - 1 ] ))
              --len;
        ...
        {
          auto initial = pos == 0 && m_stringIndex == 0 ? m_column.m_initialIndent : std::string::npos;
    ...
    public:
        ...
        auto operator* () const -> std::string
        {
          assert( m_stringIndex < m_column.m_strings.size());
          assert( pos <= m_end );
          if ( pos + m_column.m_width < m_end )
          {
            return addIndentAndSuffix( line().substr( pos, m_len ));
          }
          else
          {
            return addIndentAndSuffix( line().substr( pos, m_end - pos ));
          }
        }
...
TestCase::TestCase ( ITestInvoker* testCase, TestCaseInfo&& info ) : TestCaseInfo( std::move( info )),
                                                                     test( testCase )
{
}
...
///////////////////////////////////////////////////////////////////////////
TestInvokerAsFunction::TestInvokerAsFunction ( void(* testAsFunction) ()) noexcept : m_testAsFunction(
  testAsFunction )
{
}
  ...
  if ( m_mode == None )
  {
    switch ( c )
    {
      ...
      case '~':
        ...
        return;
      case '[':
        return startNewMode( Tag, ++pos );
      case '"':
        return startNewMode( QuotedName, ++pos );
      case '\\':
        return escape();
      default:
        startNewMode( Name, pos );
        break;
    }
  }
  if ( m_mode == Name )
...
void TestSpecParser::escape ()
{
  ...
  m_mode = EscapedName;
  m_escapeChars.push_back( pos );
}


std::string TestSpecParser::subString () const
{
  return m_arg.substr( m_start, pos - m_start );
}
